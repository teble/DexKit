...
该选项将 invoke 操作数、类定义索引缩为 16 位，将紧凑关系索引的偏移缩为 32 位，保留原来的
vector 扩容方式和行内顺序。方法自身的 ID 及反向记录中的方法身份保留 32 位，因为方法定义的
ID 可以比 invoke 指令操作数更宽。字符串 ID 和行长度仍为 32 位，因此保留 jumbo 字符串引用、
超过 65535 的方法 ID 和长列表。公开 Bean/schema 的 ID 类型不变。

累计偏移、计数必须能用 u32 表示，类定义表最多包含 65536 项；超出时触发始终启用的检查，
不会静默回绕。这些是表示范围限制，不代表原 Reader 容忍的所有非规范输入都满足新布局要求。

caller、字段 reader/writer 记录保留原来的 8 字节表示，其中源方法 ID 仍为完整的 u32。
此选项不增加方法表项数上限。
内部 C++ 布局和类型随开关改变，原生调用方必须使用一致的编译定义。Java/Kotlin API 和结果
序列化格式保持不变。
